"""Celery task: process an uploaded audio file (transcribe → chunk → embed → index)."""
...
logger = get_logger("task.audio")


@celery_processor("app.tasks.process_audio.process_audio", logger_name="task.audio")
async def process_audio(file_id: str, user_id: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = await transcription_service.transcribe_long(local_path, work_dir=tmp)
